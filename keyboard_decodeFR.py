...
def simulate_keypresses(keypresses, text_mode=True):
    ...
    for modifiers, key in keypresses:
        ...
        # Shift pressed? AltGr is used on many European keyboard layouts
        shift = 'Shift' in modifiers
        altgr = 'AltGr' in modifiers
        ...
        match key[0]:
            ...
            case _:
                if key[0].isalpha():
                    if not altgr:  # Don't apply capslock with AltGr
                        shift ^= capslock
                
                # Select the correct character based on modifiers
                if altgr and len(key) > 2:
                    char = key[2]
                elif shift and len(key) > 1:
                    char = key[1]
                elif len(key) > 0:                    char = key[0]
                else:
                    char = f'<{key[0]}>'
                
                output[line].insert(pos, char)
                pos += 1
...
=============
  MORE INFO
=============
Simulation mode will in most cases provide a similar result to the original input, but is configured for French AZERTY layout.
